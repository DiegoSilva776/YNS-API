    ...
    findAll: function (user, callback) {
        try {
            ...
            if (evalUtils.isValidVal(user)) {
                ...
                dbRef.orderByChild('user/firebaseUid').equalTo(user.firebaseUid).once("value", function(snapshot) {
    
                    try {
                        if (!evalUtils.isValidVal(snapshot)) {
                            logUtils.logMessage(TAG, `${MSG_FAILED_GET_USER_NOTIFICATION_DB} ${MSG_OR_USER_NOTIFICATIONS_ON_DB_IS_EMPTY}`);
                            handleSuccess();
                        } else {
                            snapshot.forEach(function(data) {
                                resUserNotifications.push(data.val());
                            });
